...
const documentDescriptions: Record<string, { title: string; description: string }> = {
  projectRequirements: {
    ...
    description: "Comprehensive overview of project specifications and requirements"
  },
  techStack: {
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        {documentOrder.map((docType) => (
          <Collapsible key={docType} className="border rounded-lg">
            <CollapsibleTrigger className="flex items-center justify-between w-full p-4 hover:bg-accent">
              <div className="flex items-center space-x-4">
                ...
                />
                <span className="font-medium">{documentDescriptions[docType].title}</span>
              </div>
